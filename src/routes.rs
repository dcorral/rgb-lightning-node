...
use lightning::rgb_utils::{
    read_rgb_channel_info, read_rgb_payment_info, write_rgb_channel_info, STATIC_BLINDING,
};
...
pub(crate) async fn asset_balance(
    ...
    for chan_info in unlocked_state.channel_manager.list_channels() {
        ...
        let rgb_info =
            match read_rgb_channel_info(unlocked_state.kv_store.as_ref(), &channel_id_str, false) {
                Ok(info) => info,
                Err(_) => continue,
            };
...
pub(crate) async fn list_channels(
    ...
    for chan_info in unlocked_state.channel_manager.list_channels() {
        ...
        let channel_id_str = chan_info.channel_id.0.as_hex().to_string();
        if let Ok(rgb_info) =
            read_rgb_channel_info(unlocked_state.kv_store.as_ref(), &channel_id_str, false)
        {
...
pub(crate) async fn list_payments(
    ...
    for (payment_hash, payment_info) in &inbound_payments {
        let (asset_amount, asset_id) =
            match read_rgb_payment_info(unlocked_state.kv_store.as_ref(), payment_hash, true) {
    ...
    for (payment_id, payment_info) in &outbound_payments {
        ...
        let (asset_amount, asset_id) =
            match read_rgb_payment_info(unlocked_state.kv_store.as_ref(), payment_hash, false) {
                Ok(info) => (Some(info.amount), Some(info.contract_id.to_string())),
                Err(_) => (None, None),
            };
...
pub(crate) async fn get_payment(
    ...
    for (payment_hash, payment_info) in &inbound_payments {
        if payment_hash == &requested_ph {
            let (asset_amount, asset_id) =
                read_rgb_payment_info(unlocked_state.kv_store.as_ref(), payment_hash, true)
                    .ok()
    ...
    for (payment_id, payment_info) in &outbound_payments {
        ...
        if payment_hash == &requested_ph {
            let (asset_amount, asset_id) =
                read_rgb_payment_info(unlocked_state.kv_store.as_ref(), payment_hash, false)
                    .ok()
                    .map(|info| (Some(info.amount), Some(info.contract_id.to_string())))
                    .unwrap_or((None, None));
